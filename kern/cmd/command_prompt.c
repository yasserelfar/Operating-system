...
int process_command(int number_of_arguments, char** arguments)
{
	...
	bool found = 0;
	bool rightNumberOfArguments = 0;
	int commandNumberOfArguments;
	int commandIndex;
	...
	{
		if (strlen(arguments[0]) == strlen(commands[i].name)
			&& strncmp(arguments[0],commands[i].name, strlen(arguments[0]))== 0)
		{
			found = 1;
			commandNumberOfArguments = commands[i].num_of_args;
			commandIndex = i;
			break;
		}
	}
	if (found == 1 && (commandNumberOfArguments == number_of_arguments - 1 ||
												(commandNumberOfArguments == -1
												&& number_of_arguments > 1)))
		{
			return commandIndex;
		}
	else if (found == 1 && commandNumberOfArguments != number_of_arguments - 1)
		{
			LIST_INSERT_HEAD(&foundCommands, &commands[commandIndex]);
	...
	for(int i = 0; i < NUM_OF_COMMANDS; i++)
	{
		...
		char* x = NULL;
		char* y = NULL;
		for (int j = 0; j < strlen(arguments[0]) + 1; j++)
		{
			x = strfind(commands[i].name, arguments[0][j]);
			if (x > y && x != '\0')
			{
				y = x;
			}
