...
uint32 is_this_frame_marked(uint32 va){

	//cprintf("in is marked \n");
	...
	uint32 entry = (uint32) ptr_page_table[PTX(va)];

//
//	cprintf("this is marked: %d, is present: %d, va: %p \n",
//			((entry&PERM_MARKED) == PERM_MARKED),
//			((entry&PERM_PRESENT) == PERM_PRESENT), (void*)va);

	uint32 is_marked_bool = ( entry&PERM_MARKED ) == PERM_MARKED;
	return is_marked_bool;
}
...
//	return (void*)-1 ;
	 ...
	 */
		...
		if(increment==0)
		{
			return (void*) env->segment_U_break;
		}
		else if(increment>0)
			{

				uint32 new_increment=ROUNDUP(increment,PAGE_SIZE);
	 			if((new_increment+env->segment_U_break)>env->hard_U_limit){
					return (void*) -1;
	 			}
				uint32 return_address=env->segment_U_break;
				env->segment_U_break+=new_increment;

				for(uint32 i=return_address;i<env->segment_U_break;i+=PAGE_SIZE)
				{
					uint32 * ptrtable=NULL;
						int cheak=	get_page_table(env->env_page_directory,i,&ptrtable);
						if(ptrtable==NULL){
							ptrtable= create_page_table(env->env_page_directory,i);

						}
						ptrtable[PTX(i)]=ptrtable[PTX(i)]|PERM_MARKED|PERM_USER|PERM_WRITEABLE;
				}
			...
			for(uint32 i=env->segment_U_break-increment;i<=env->segment_U_break;i+=PAGE_SIZE)
			{
				pt_set_page_permissions(env->env_page_directory,i,0,PERM_MARKED);

			}
			env->segment_U_break-=increment;
			return (void *) env->segment_U_break;
		}


}
uint32 sys_size(uint32 vir){
	uint32 *new_page=NULL;

	int page=get_page_table(curenv->env_page_directory,vir,&new_page);
	if(new_page==NULL)
		{

		return 0;
		}
	struct FrameInfo* ptr_frame_info = get_frame_info(curenv->env_page_directory,vir, &new_page);
	int num_frams=ptr_frame_info->size;
	return (uint32)num_frams;
}
/**************************************************************************/
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_sbrk:
//		if ( a1 > USER_LIMIT || (void*)a1 == NULL)
//		{
//			cprintf("in sbrk kill\n");
...
//		}
		...
		break;
	case SYS_perm:
			return (uint32)pt_get_page_permissions(curenv->env_page_directory, a1);
			break;

	case SYS_hard_limit:
		return  curenv->hard_U_limit;
		break;
	case SYS_current_env:
	...
	case SYS_env_set_nice:
		...
		break;
	case SYS_set_perm:
		pt_set_page_permissions(curenv->env_page_directory,a1,0,0x400);
		break;
...
//				||PAGE_SIZE < a2
			...
			//	a1 + KERNEL_BASE >= USER_LIMIT || a2 + KERNEL_BASE >= USER_LIMIT)
...
//			|| PAGE_SIZE < a2
	...
	//=====================================================================
	case SYS_is_marked:
		return is_this_frame_marked(a1);
		break;
