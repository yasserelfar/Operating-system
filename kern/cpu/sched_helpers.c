...
void env_set_nice(struct Env* e, int nice_value)
{
	...
	if(e->env_status==ENV_NEW){
			...
			fixed_point_t recentover = fix_unscale(e->recent_cpu,4);
			fixed_point_t niceby = fix_int(e->nice * 2);
			fixed_point_t muinus = fix_sub(max,recentover);
			fixed_point_t muinus2 = fix_sub(muinus,niceby);
			e->priority=fix_trunc(muinus2);
			if(e->priority<PRI_MIN)
			{
				e->priority=PRI_MIN;
			}
			else if(e->priority>PRI_MAX)
			{
				e->priority=PRI_MAX;
			}
		}
//	if(e->nice<NICE_MIN||e->nice>NICE_MAX){
//		printf("invalid nice value it should between -20&20 \n");
//		return;
//	}
//	for(int i=0;i<num_of_ready_queues;i++){
//		ready_process+= env_ready_queues[i].size;//lazm a3ml lkol queue ashof ready process in it
//		if(timer_ticks()%(1000/quantums[i])==0)
//		{
//
//			load_avg=fix_add((fix_mul(fix_int(59/60),load_avg)),(fix_mul(fix_int(1/60),fix_int(ready_process))));
//			 struct Env_Queue* My_queue = &env_ready_queues[i];
//			struct Env* env = My_queue->lh_first;
//			while (env!=NULL)
//			{
//				//calc recent_cpu
//				e->recent_cpu=fix_round(fix_scale((fix_div((fix_mul(load_avg,fix_int(2))),(fix_add((fix_mul(load_avg,fix_int(2))),fix_int(1))))),((e->recent_cpu)+e->nice)));
//				//calc priority
//				if(timer_ticks()%(4*quantums[i])==0)
//				{
//						e->priority=fix_round(fix_sub((fix_sub((fix_int(PRI_MAX)),fix_int((e->recent_cpu)/4))),(fix_int(e->nice*2))));
//						if(e->priority<PRI_MIN)
//						{
//							e->priority=PRI_MIN;
//						}
...
//						}
//				}
//				env=env->prev_next_info.le_next;
//			}
//
//		}
//	}
...
//		load_avg=fix_add((fix_mul(fix_int(59/60),load_avg)),(fix_mul(fix_int(1/60),fix_int(ready_process))));
//
//		if(ticks%(1000/quantums[])==0){
//			e->recent_cpu=fix_round(fix_scale((fix_div((fix_mul(load_avg,fix_int(2))),(fix_add((fix_mul(load_avg,fix_int(2))),fix_int(1))))),((e->recent_cpu)+e->nice)));
//		}
//	}

//lazm a3ml enqueue to new environment after calc piriority
